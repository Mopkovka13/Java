...
> [Index](../../Index.md) · рядом: [Thread coordination patterns](Thread%20coordination%20patterns.md) · [Thread cancellation](Thread%20cancellation.md) · [happens-before](happens-before.md) · аббревиатуры → [Concurrency glossary](Concurrency%20glossary.md)

# `wait` / `notify`

Низкоуровневый примитив. На прикладе обычно прячется за `Lock`+`Condition` или `j.u.c.`-примитивами ([Thread coordination patterns](Thread%20coordination%20patterns.md)).
...
- `wait(timeout)` реагирует на [interrupt](Thread%20cancellation.md) — кидает `InterruptedException`.
